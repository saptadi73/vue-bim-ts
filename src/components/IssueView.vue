<template>
  <div>
    ...
    <div class="bg-white border shadow-sm rounded-xl mt-2">
      ...
      <div id="card1" class="">
        <div class="p-4 md:p-5">
          <h3 class="text-lg font-bold text-gray-800">Activities List</h3>
          <div class="w-[80vw] h-auto rounded-md">
            <div>
              <div id="hs-datatable-filter" class="flex flex-col">
                      ...
                      <table class="min-w-full">
                        <thead class="border-y border-gray-200">
                          <tr>
                            ...
                            <th class="py-3 px-4 text-xs font-Roboto">User</th>
                          </tr>
                        </thead>
                        <tbody class="divide-y divide-gray-200">
                          ...
                          >
                            ...
                            <td class="p-4 text-xs font-medium text-gray-800">
                              {{ formatDateTime(user.date_activity) }}
                            </td>
                            ...
                            <td class="font-medium text-gray-800 font-Roboto">
                              <div class="flex items-center p-2">
                                ...
                                <div class="text-xs ml-2">{{ user.name }}</div>
                              </div>
                            </td>
                          </tr>
                        </tbody>
                ...
                <div class="flex items-center mt-4">
                  <div class="text-blue-600">
                    Showing <span>{{ filteredUsers.length }}</span> activities
                  </div>
                </div>
              </div>
            </div>
        ...
        <div
          class="bg-gray-100 border-t rounded-b-xl py-3 px-4 md:py-4 md:px-5"
        >
          <p class="mt-1 text-sm text-gray-500">Activities List</p>
        </div>
      </div>
    </div>
    <div>
      <div v-if="fotoDisplay" class="fixed inset-0 bg-black bg-opacity-75 z-40 flex items-center justify-center p-4">
        <button 
          @click="closeFoto"
...
const users = ref<IssueActivities[]>([]);

interface IssueActivities {
  id: number;
...
const hasilIssueActivities = ref<IssueActivities[]>([]);
const filteredUsers = computed<IssueActivities[]>(() => {
  if (!searchQuery.value) {
    return hasilIssueActivities.value;
  }
  return hasilIssueActivities.value.filter(
    (user) =>
      user.activity.toLowerCase().includes(searchQuery.value.toLowerCase()) ||
      user.name.toLowerCase().includes(searchQuery.value.toLowerCase()) ||
      user.date_activity.toLowerCase().includes(searchQuery.value.toLowerCase())
  );
});
...
onMounted(async () => {
  ...
  async function getDataIssueActivities() {
    const expressid = parseInt(route.params.id as string);
    const url = `${BASE_URL}issue/activities/${expressid}`;
    const getTaskActivities = await axios.get(url);
    console.log("Data TaskActivities: ", getTaskActivities.data);
    hasilIssueActivities.value = getTaskActivities.data.result;
  }

  getLocation();
  getDataIssueActivities();
