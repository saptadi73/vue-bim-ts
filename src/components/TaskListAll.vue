              ...
              <div id="hs-datatable-filter" class="flex flex-col">
                      ...
                      <table class="min-w-full">
                        <thead class="border-y border-gray-200">
                          <tr>
                            ...
                            <th class="py-3 px-4 text-xs font-Roboto">Value</th>
                        ...
                        <tbody class="divide-y divide-gray-200">
                          ...
                          >
                            <u
                              ><button @click="goToURL(`/project/task/list${user.location}/${user.project_id}`)">{{user.name}}</button></u
                            >
                          </td>
                            <td class="p-4 text-xs font-medium text-gray-800">
                          ...
                          >
                            <u
                              ><RouterLink :to="`/project/task/view${user.location}/${user.project_id}`">Activities</RouterLink></u
                            >
                          </td>
                          </tr>
                        </tbody>
                ...
                <div class="flex items-center mt-4">
                  <div class="text-blue-600">
                    Showing <span>{{ filteredUsers.length }}</span> Tasks
                  </div>
                </div>
              </div>
            </div>
  ...
  return hasilTaskActivities.value.filter(
    (user) =>
      ...
      formatDateTime(user.end_date)
        .toLowerCase()
        .includes(searchQuery.value.toLowerCase()) ||
      user.issued_by.toLowerCase().includes(searchQuery.value.toLowerCase()) ||
      user.assign_to.toLowerCase().includes(searchQuery.value.toLowerCase()) ||
      formatRupiah(user.value)
        .toLowerCase()
        .includes(searchQuery.value.toLowerCase())
  );
...
onMounted(async () => {
  ...
  async function getDataTaskList() {
    ...
    const url = `${BASE_URL}tasks/all/${expressid}`;
